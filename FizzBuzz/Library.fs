...
module Application =
    ...
    let execute =
        Domain.execute Parser.tryParse Validator.ValidNumber.isValidNumber FizzBuzz.getFizzBuzzString

    let application (input: Input) (output: Output) =
        fun () ->
            output "Please enter a valid integer between 1 and 4000:"

            input ()
            |> execute
            |> function
                | Ok s -> $"Here is the output:\n%s{s}" |> output
                | Error (ParserError (NotANumber s)) -> $"%s{s} is not a valid integer." |> output
                | Error (ValidatorError (InvalidNumber n)) ->
                    $"You entered %i{n}. Please enter a number between 1 and 4000."
                    |> output
